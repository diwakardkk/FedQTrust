...
def subtract_state(local: StateDict, global_state: StateDict) -> StateDict:
    return {
        k: local[k].detach() - global_state[k].detach().to(device=local[k].device, dtype=local[k].dtype)
...
def add_delta(global_state: StateDict, delta: StateDict) -> StateDict:
    return {
        k: global_state[k].detach() + delta[k].detach().to(device=global_state[k].device, dtype=global_state[k].dtype)
        for k in global_state
    }
